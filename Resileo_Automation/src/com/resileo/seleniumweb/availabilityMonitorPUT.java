...
public class availabilityMonitorPUT extends ReusableMethods {
	...
	public void createAVM() {
		try {
			test = extent.startTest("Availability Monitoring PUT - Add","Availability Monitoring Module Test-Req Method :PUT ");
			...
			// click on Request Method PUT in the Drop down list
			WaitforObject(appedo_avm_requestMethodPUT);
			browser.findElement(getObject(appedo_avm_requestMethodPUT)).click();
			...
			browser.findElement(getObject(appedo_avm_ReqBodyTxt))
					.sendKeys("<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\r\n" + "<name>morpheus</name>\r\n"
			...
			Thread.sleep(2000);
			browser.findElement(getObject(appedo_avm_resBodyPUT)).click();
			...
			// Status Code
			String statuscode = browser.findElement(getObject(appedo_avm_resStatuscode)).getText().toString();
			test.log(LogStatus.PASS, "Status Code : " + statuscode);

			// Status
			String status = browser.findElement(getObject(appedo_avm_putStatus)).getText().toString();
	...
	public void EditAVMModule() {
		try {
			test = extent.startTest("Availability Monitoring PUT - Edit", "Edit AVM module");
			// Click on Edit icon
			Thread.sleep(2000);
			WaitforObject(appedo_rum_Edit);
	...
	public void DeleteAVMModule() {
		try {
			test = extent.startTest("Availability Monitoring PUT - Delete", "Delete AVM Test");
