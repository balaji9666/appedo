...
public class availabilityMonitorWSDL extends ReusableMethods {
	...
	public void createAVM() {
		try {
			...
			String URL1 = "http://demo-apps.appedo.com/eComResileo/services/ParcelServiceImpl";
			WaitforObject(appedo_avm_txtTestUrl);
			browser.findElement(getObject(appedo_avm_txtTestUrl)).clear();
			...
			WaitforObject(appedo_avm_reqMethodWsdl);
			browser.findElement(getObject(appedo_avm_reqMethodWsdl)).click();

			// Request Body content text/html
			Thread.sleep(5000);
			WaitforObject(appedo_avm_ReqBodyTxt);
			browser.findElement(getObject(appedo_avm_ReqBodyTxt)).sendKeys("<Envelope xmlns=\"http://schemas.xmlsoap.org/soap/envelope/\">\r\n" + 
					"<Body>\r\n" + 
					"<getTrackingIdStatus xmlns=\"http://service.ws.ecom_sample.resileo.com\">\r\n" + 
					"<trackId>AE00301</trackId>\r\n" + 
					"</getTrackingIdStatus>\r\n" + 
					"</Body>\r\n" + 
					"</Envelope>");

			// Click on Header
			browser.findElement(getObject(appedo_avm_header)).click();
			
			// Expand Header
			browser.findElement(getObject(appedo_avm_expandHeader)).click();
			
			// Remove headers
			WaitforObject(appedo_avm_removeHeaderAccept);
			browser.findElement(getObject(appedo_avm_removeHeaderAccept)).click();
			Thread.sleep(1000);
			browser.findElement(getObject(appedo_avm_removeHeaderAcceptLanguage)).click();
			Thread.sleep(1000);
			browser.findElement(getObject(appedo_avm_removeHeaderConnection)).click();
			Thread.sleep(1000);
			browser.findElement(getObject(appedo_avm_removeHeaderUserAgent)).click();
			Thread.sleep(1000);
			
			// Add Headers
			browser.findElement(getObject(appedo_avm_addValueContentType)).clear();
			browser.findElement(getObject(appedo_avm_addValueContentType)).sendKeys("text/xml;charset=UTF-8");
			WaitforObject(appedo_avm_addHeader);
			browser.findElement(getObject(appedo_avm_addHeader)).click();
			browser.findElement(getObject(appedo_avm_addAcceptEncoding)).sendKeys("Accept-Encoding");
			browser.findElement(getObject(appedo_avm_addValueAcceptEncoding)).sendKeys("gzip,deflate");
			WaitforObject(appedo_avm_addHeader);
			browser.findElement(getObject(appedo_avm_addHeader)).click();
			browser.findElement(getObject(appedo_avm_addSoapAction)).sendKeys("SOAPAction");
			browser.findElement(getObject(appedo_avm_addValueSoapAction)).sendKeys("\"\"");
			...
			browser.findElement(getObject(appedo_avm_txtMinBreachCnt)).clear();
			browser.findElement(getObject(appedo_avm_txtMinBreachCnt)).sendKeys("2");
